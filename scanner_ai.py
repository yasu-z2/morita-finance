...
# バージョン: 3.2.4 (Gemini 2.5 Flash + 外部リンク統合)
# ==========================================================
...
def run_scanner_final():
        ...
        try:
            ...
            if is_range_s1 and up_from_low and high_vol_s1 and ((current_price * 100) <= PRICE_LIMIT_YEN):
                ...
                item = {
                    "コード": pure_code, 
                    "名称": name, 
                    "終値": round(float(current_price), 1),
    ...
    mail_body = f"■ 株価選別W分析レポート v3.2.4 (Gemini 2.5 Flash)\n実行時間: {elapsed}分 / 抽出数: {len(stage1_list)}件\n\n"
    ...
    mail_body += "【参考：本日抽出銘柄のチャート一覧】\n"
    for s in top_picks[:10]:
        mail_body += f"- {s['名称']}({s['コード']}): {s['URL_K']}\n"
