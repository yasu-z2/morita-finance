...
RANGE_FACTOR_S2 = 1.10        # 第二段階：底値圏レンジ (厳格)
...
def run_scanner_final_v321():
    ...
    analysis_1 = call_gemini(f"プロの証券アナリストとして、以下の銘柄の反発要因を市場全体と関連付けて分析してください。\n{summary_data}")
    
    detail_data = "\n".join([f"{s['名称']}({s['コード']}): 終値{s['終値']}, 指値1:{s['第1指値']}, 損切:{s['損切目安']}" for s in top_picks[:5]])
    analysis_2 = call_gemini(f"以下の厳選銘柄に対して、具体的な期待値と損切り判断基準を提示してください。\n{detail_data}")

    # --- メール本文の構築 ---
    elapsed = round((time.time() - start_time) / 60, 1)
    mail_body = f"■ 株価選別W分析レポート v3.2.1\n実行時間: {elapsed}分 / 抽出数: {len(stage1_list)}件\n\n"
